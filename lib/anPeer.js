...
// Анализируем
  ...
  if ( msg.m.match('называют Хламосборщик.')) {
    st.act        = true
    st.goodMob2    = true
  }
  if ( msg.m.match('Сделка завершена.')) {
  ...
  // Ловим лечение
  parse = msg.m.match(/❤️(\d+)\\/)
  if (parse) st.healthLvl  = Number(parse[1])
  parse = msg.m.match(/❤️(\d+)\//)
  if (parse) st.healthLvl  = Number(parse[1])
  ...
  if (parse = msg.m.match(/Уровень голода: (\d+)%/))
    st.hangryLvl  = Number(parse[1])
